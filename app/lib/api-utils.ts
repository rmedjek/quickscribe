...
function isRetryableNetworkError(error: any): boolean {
  ...
  if (retryableErrorCodes.includes(errorCode)) {
    console.log(
      `[RetryUtil] Detected retryable network error code: ${errorCode}`
    );
    return true;
  }
  return false;
...
function isRetryableGroqError(error: any): boolean {
  ...
  if (error instanceof Groq.APIError) {
    ...
    if (
      error.status &&
      (retryableStatusCodes.includes(error.status) || error.status >= 500)
    ) {
      console.log(
        `[RetryUtil] Detected retryable Groq.APIError status code: ${error.status}`
      );
      return true;
    }
  }
...
export async function retryWithBackoff<T>({
  ...
  while (attempts <= maxRetries) {
    ...
    try {
      ...
      console.warn(
        `[RetryUtil] Operation "${operationName}" attempt ${attempts} failed. Error: ${
      ...
      if (attempts > maxRetries || !shouldRetry) {
        console.error(
          `[RetryUtil] Max retries (${maxRetries}) reached or error is not retryable for "${operationName}". Giving up.`
        );
      ...
      const waitTime = Math.min(
        Math.round(currentBackoffMs + jitter),
        maxBackoffMs
      );

      console.log(
        `[RetryUtil] Retrying operation "${operationName}" in ${waitTime}ms... (Backoff: ${Math.round(
          currentBackoffMs
        )}ms, Jitter: ${Math.round(jitter)}ms)`
      );
  ...
  console.error(
    `[RetryUtil] Exited retry loop unexpectedly for "${operationName}".`
  );
  throw lastError;
}
