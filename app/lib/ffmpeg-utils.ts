...
export async function getFFmpegInstance(
  ...
  if (
    ffmpegInstance &&
    typeof ffmpegInstance.isLoaded === "function" &&
  ...
  instance.on("log", (logData) => {
    // With overloads, logData should be FFmpegLogData
    ...
    if ("message" in logData && "type" in logData) {
      ...
      console.warn(
        '[ffmpeg-utils] Received unexpected data structure on "log" event:',
        logData
      );
    }
  });
  ...
  if (progressCallback) {
    instance.on("progress", (progressData) => {
      // progressData should be FFmpegProgressData
      ...
      if (
        ...
        console.warn(
          '[ffmpeg-utils] Received unexpected data structure on "progress" event:',
          progressData
        );
      }
    });
  ...
  console.log(
    "[ffmpeg-utils] Attempting to load FFmpeg UMD core from /public/ffmpeg-core-umd/"
  );
  try {
    await instance.load({
...
export async function extractAudio({
  ...
  console.log(
    `[extractAudio] Using provided FFmpeg instance: ${isProvidedFfmpegReady}`
  );
  ...
  if (
    !ffmpegToUse ||
    (typeof ffmpegToUse.isLoaded === "function" && !ffmpegToUse.isLoaded())
  ) {
    throw new Error(
      "[extractAudio] FFmpeg instance is not available or not loaded."
    );
  }
  ...
  let operationProgressListener: ((data: FFmpegProgressData) => void) | null =
    null;
  ...
  console.log(
    `[extractAudio] Writing file "${inputFileName}" to FFmpeg's virtual file system...`
  );
  ...
  switch (outputFormat) {
    ...
    case "mp3":
      ...
      break;
    case "flac":
      args.push("-acodec", "flac", "-ar", "16000", "-ac", "1");
      break;
    case "wav":
    ...
    default:
      throw new Error(
        `[extractAudio] Unsupported output format: ${outputFormat}`
      );
  }
  args.push(outputFileName);
  console.log(
    `[extractAudio] Running FFmpeg command: ffmpeg ${args.join(" ")}`
  );
  ...
  try {
    ...
    if (exitCode !== 0) {
      ...
      if (exitCode === 1) {
        errorMessage +=
          " This commonly indicates issues like unsupported input format, file corruption, no audio track, or internal FFmpeg errors.";
      }
    ...
    if (data.length === 0) {
      throw new Error(
        "[extractAudio] Extracted audio is empty. The video might not have an audio track or the format is unsupported."
      );
    }
    ...
    if (ffmpegToUse.FS && typeof ffmpegToUse.FS === "function") {
      try {
        console.log(
          `[extractAudio] Unlinking "${inputFileName}" and "${outputFileName}" from FFmpeg FS...`
        );
        ...
        console.warn(
          "[extractAudio] Could not unlink files from FFmpeg FS:",
          fsError
        );
      }
    } else {
      console.warn(
        "[extractAudio] ffmpeg.FS method not available for cleanup."
      );
    }

    console.log(
      `[extractAudio] Extracted audio file "${outputFileName}" with size: ${data.length} bytes.`
    );
    ...
    // }
    console.error(
      "[extractAudio] Error during FFmpeg execution or post-processing:",
      error
    );
    throw error;
  }
