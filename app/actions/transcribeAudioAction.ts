...
import {TRANSCRIPTION_MODELS, TranscriptionMode} from "@/types/app";
...
export async function transcribeAudioAction(
  ...
  try {
    ...
    console.log(
      `[Server Action] transcribed with actual model: "${modelToUse}"`
    );
    ...
    console.log(
      "[Server Action] Raw verbose_json response from Groq (first 500 chars):",
      JSON.stringify(transcription, null, 2).substring(0, 500) + "..."
    );
    ...
    if (rawText && Array.isArray(segmentsFromApi)) {
      const typedSegments: Segment[] = segmentsFromApi.map(
        (
      ...
      console.error(
        "[Server Action] Groq response did not contain expected fields:",
        transcription
      );
      return {
        success: false,
    ...
    if (typeof error === "object" && error !== null) {
      ...
      userFriendlyError = `An unexpected error occurred during transcription: ${String(
        error
      )}`;
    }
    ...
    if (error instanceof Groq.APIConnectionTimeoutError) {
      userFriendlyError = `Groq API Error: Connection timed out after ${
        GROQ_REQUEST_TIMEOUT_MS / 1000
      }s. The audio file might be too large or processing took too long.`;
    } else if (error instanceof Groq.APIError) {
      if (error.status === 503) {
        userFriendlyError =
          "Groq’s transcription service is temporarily unavailable (503). This usually means the service is very busy or undergoing maintenance. Please try again in a few minutes.";
        ...
        userFriendlyError = `Groq API Error (Status: ${
          error.status || "N/A"
        }): ${error.message}`;
      }
      console.error(
        "[Server Action] Full Groq.APIError object:",
