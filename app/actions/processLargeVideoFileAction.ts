...
export async function processLargeVideoFileAction(
  ...
  console.log(`[LargeFileAction] Received video file: ${videoFile.name}, size: ${videoFile.size}, type: ${videoFile.type}`);
  ...
  try {
    ...
    const tempVideoFileName = sanitizeFilename(`temp_uploaded_video_${uniqueId}.${originalExtension}`);
    ...
    console.log(`[LargeFileAction] Uploaded video saved successfully to: ${tempVideoPath}`);

    // --- 2. Use FFmpeg on the server to extract audio ---
    const audioOutputFormat = 'opus';
    const audioFileName = sanitizeFilename(`extracted_audio_large_${uniqueId}.${audioOutputFormat}`);
    ...
    if (stderr) console.log(`[LargeFileAction] FFmpeg stderr (often informational): ${stderr}`);
    ...
    try {
        ...
        console.log(`[LargeFileAction] Audio extracted to: ${tempAudioPath}, size: ${stats.size}`);
    } catch (statError: any) {
        console.error(`[LargeFileAction] Error accessing extracted audio file stats: ${statError.message}`);
    ...
    if (tempVideoPath) {
      console.log(`[LargeFileAction] Attempting to delete temp video: ${tempVideoPath}`);
      fsPromises.unlink(tempVideoPath)
        .then(() => console.log(`[LargeFileAction] Deleted temp video: ${tempVideoPath}`))
        .catch(err => console.warn(`[LargeFileAction] Failed to delete temp video (${tempVideoPath}):`, err.message));
    }
    if (tempAudioPath) {
      console.log(`[LargeFileAction] Attempting to delete temp audio: ${tempAudioPath}`);
      fsPromises.unlink(tempAudioPath)
        .then(() => console.log(`[LargeFileAction] Deleted temp audio: ${tempAudioPath}`))
        .catch(err => console.warn(`[LargeFileAction] Failed to delete temp audio (${tempAudioPath}):`, err.message));
    }
  }
}