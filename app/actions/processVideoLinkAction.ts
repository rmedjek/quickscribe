...
import {TranscriptionEngine, TranscriptionMode} from "@/types/app";

import {transcribeWithAssemblyAiAction} from "./transcribeWithAssemblyAiAction";
...
export async function processVideoLinkAction(
  ...
  try {
    ...
    if (url.hostname.includes("youtube.com") && url.pathname === "/results") {
      const errorMsg =
        "YouTube search result pages are not supported. Please provide a link to a single video.";
      console.warn(
        `[VideoLinkAction] Aborting due to search results URL: ${videoUrl}`
      );
  ...
  try {
    console.log(
      "[VideoLinkAction] Preparing to download video/audio content..."
    );
    ...
    try {
      ...
      if (
        /\.(mp4|mov|webm|avi|mkv|flv|mpeg|mpg|wmv|m4a|aac|ogg|wav|flac|opus)$/i.test(
          parsedUrl.pathname
        )
      ) {
      ...
      console.warn(
        "[VideoLinkAction] URL parsing failed, will attempt download with yt-dlp:",
        e
      );
      // If URL is malformed, yt-dlp will likely fail, which is desired.
    }

    tempDownloadedFilePath = path.join(
      os.tmpdir(),
      `${downloadedFileNameBase}.${downloadedFileExtension}`
    );

    if (isDirectKnownVideoFileLink) {
      console.log(
        `[VideoLinkAction] Detected direct file link: ${videoUrl}. Attempting download with fetch...`
      );
      const response = await fetch(videoUrl);
      if (!response.ok || !response.body) {
        throw new Error(
          `Failed to download from direct link: ${response.status} ${response.statusText} for URL: ${videoUrl}`
        );
      }
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        response.body!.on("error", (err) => {
          console.error(
            "[VideoLinkAction] Direct link response body stream error:",
            err
          );
          reject(err);
        });
        fileStream.on("finish", () => {
          console.log(
            "[VideoLinkAction] Direct link stream piped to file successfully."
          );
          resolve();
        });
        fileStream.on("error", (err) => {
          console.error("[VideoLinkAction] Direct link fileStream error:", err);
          reject(err);
        });
      });
    } else {
      ...
      console.log(
        `[VideoLinkAction] URL (${videoUrl}) is not a clear direct file link or failed parsing. Attempting download with yt-dlp CLI...`
      );
      ...
      tempDownloadedFilePath = path.join(
        os.tmpdir(),
        `${downloadedFileNameBase}.${ytDlpOutputExtension}`
      );
      ...
      try {
        const {stdout: ytStdout, stderr: ytStderr} = await execAsync(
          ytDlpCommand,
          {timeout: 300000}
        ); // 5 min timeout

        if (ytStdout)
          console.log(`[VideoLinkAction] yt-dlp stdout: ${ytStdout}`);
        if (ytStderr)
          console.log(
            `[VideoLinkAction] yt-dlp stderr (often progress/info): ${ytStderr}`
          );

        // Check if the file was actually created by yt-dlp
        await fsPromises.stat(tempDownloadedFilePath);
        console.log(
          `[VideoLinkAction] Content successfully downloaded by yt-dlp to: ${tempDownloadedFilePath}`
        );
      } catch (ytDlpError: any) {
        console.error(`[VideoLinkAction] yt-dlp execution error:`, ytDlpError);
        const errorMessage = ytDlpError.stderr
          ? `yt-dlp failed: ${ytDlpError.stderr}`
          : `yt-dlp failed: ${ytDlpError.message}`;
    ...
    if (!tempDownloadedFilePath) {
      ...
      throw new Error(
        "Download path was not set, indicating a download failure or logic error."
      );
    }
    console.log(
      `[VideoLinkAction] Content successfully downloaded to: ${tempDownloadedFilePath}`
    );
    ...
    const opusAudioFileName = sanitizeFilename(
      `extracted_audio_opus_${uniqueId}.${audioOutputFormat}`
    );
    ...
    if (stderr)
      console.log(
        `[VideoLinkAction] FFmpeg stderr (often informational): ${stderr}`
      );
    ...
    try {
      ...
      if (stats.size === 0) {
        throw new Error(
          `Extracted Opus audio file is empty. FFmpeg stderr (if any): ${
            stderr || "N/A"
          }`
        );
      }
      console.log(
        `[VideoLinkAction] Opus audio extracted to: ${tempOpusAudioPath}, size: ${stats.size}`
      );
    } catch (statError: any) {
      console.error(
        `[VideoLinkAction] Error accessing extracted Opus audio file stats: ${statError.message}`
      );
      throw new Error(
        `Extracted Opus audio file not found or unreadable after FFmpeg. FFmpeg stderr (if any): ${
          stderr || "N/A"
        }`
      );
    }
    ...
    if (audioFileBuffer.length > GROQ_AUDIO_LIMIT_BYTES && engine === "groq") {
      ...
      if (tempDownloadedFilePath)
        await fsPromises.unlink(tempDownloadedFilePath).catch(console.warn);
      if (tempOpusAudioPath)
        await fsPromises.unlink(tempOpusAudioPath).catch(console.warn);
    ...
    if (engine === "assembly") {
      ...
      // Default to Groq, which uses the mode (core/turbo)
      return await transcribeAudioAction(formData, mode); // Use the direct import name
    }
  } catch (error: any) {
    ...
    if (tempDownloadedFilePath) {
      console.log(
        `[VideoLinkAction] Attempting to delete temp downloaded file: ${tempDownloadedFilePath}`
      );
      fsPromises
        .unlink(tempDownloadedFilePath)
        .then(() =>
          console.log(
            `[VideoLinkAction] Deleted temp downloaded file: ${tempDownloadedFilePath}`
          )
        )
        .catch((err) =>
          console.warn(
            `[VideoLinkAction] Failed to delete temp downloaded file (${tempDownloadedFilePath}):`,
            err.message
          )
        );
    }
    if (tempOpusAudioPath) {
      console.log(
        `[VideoLinkAction] Attempting to delete temp Opus audio: ${tempOpusAudioPath}`
      );
      fsPromises
        .unlink(tempOpusAudioPath)
        .then(() =>
          console.log(
            `[VideoLinkAction] Deleted temp Opus audio: ${tempOpusAudioPath}`
          )
        )
        .catch((err) =>
          console.warn(
            `[VideoLinkAction] Failed to delete temp Opus audio (${tempOpusAudioPath}):`,
            err.message
          )
        );
    }
