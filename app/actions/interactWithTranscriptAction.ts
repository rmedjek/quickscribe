...
if (!process.env.GROQ_API_KEY) {
  throw new Error(
    "GROQ_API_KEY environment variable is not set. This is a server configuration issue."
  );
}
...
function truncateStringToTokenLimit(
  text: string,
  limit: number,
  encoding: Tiktoken
): {truncatedText: string; tokenCount: number; wasTruncated: boolean} {
  let currentText = text;
  let tokens = encoding.encode(currentText);
  let wasTruncated = false;
  const charsPerTokenEstimate = 2;
  while (tokens.length > limit) {
    wasTruncated = true;
    const tokensOver = tokens.length - limit;
    const charsToRemove = Math.max(
      1,
      Math.floor(tokensOver * charsPerTokenEstimate * 0.5)
    );
    if (currentText.length <= charsToRemove) {
      currentText = "";
      break;
    }
...
export async function interactWithTranscriptAction(
  ...
  const {transcriptText, taskType, customPrompt, llmModel, outputLanguage} =
    params;
  const modelToUse = llmModel || DEFAULT_LLM_MODEL;

  console.log(
    `[AI Action] Called for task: "${taskType}". Model: "${modelToUse}". Custom Prompt: ${
      customPrompt ? `"${customPrompt.substring(0, 50)}..."` : "N/A"
    }`
  );
  ...
  if (!transcriptText || transcriptText.trim() === "") {
    ...
    return Response.json(
      {success: false, error: "Transcript text cannot be empty."},
      {status: 400}
    );
  }
  ...
  switch (taskType) {
    ...
    case "custom_question":
      if (!customPrompt || customPrompt.trim() === "") {
        console.error(
          "[AI Action] Error: Question is empty for 'custom_question' task."
        );
        return Response.json(
          {success: false, error: "Question cannot be empty for the Q&A task."},
          {status: 400}
        );
      }
      systemPrompt = `You are an AI assistant. Your task is to answer the question based *solely* on the provided transcript. If the answer is not in the text, state that clearly. Do not make up information. Be concise. ${languageInstruction}`;
      break;
    ...
    case "identify_topics":
      systemPrompt = `You are a helpful AI assistant. Your task is to analyze the following transcript and identify the main topics or subjects discussed. " +
    ...
    default:
      ...
      return Response.json(
        {success: false, error: `Unsupported AI task type: ${taskType}`},
        {status: 400}
      );
  }
  ...
  try {
    ...
    return Response.json(
      {
        success: false,
        error: "Internal server error: Failed to initialize token counter.",
      },
      {status: 500}
    );
  }
  ...
  const modelContextLimit =
    MODEL_CONTEXT_WINDOWS[modelToUse] || DEFAULT_CONTEXT_WINDOW;
  ...
  const availableTokensForUserPayload =
    modelContextLimit -
    systemPromptTokens -
    TOKENS_RESERVED_FOR_OUTPUT_AND_TPM_BUFFER;
  ...
  if (availableTokensForUserPayload <= 100) {
    ...
    if (
      encoding.encode(transcriptText).length + systemPromptTokens >
      modelContextLimit - TOKENS_RESERVED_FOR_OUTPUT_AND_TPM_BUFFER
    ) {
  ...
  if (taskType === "custom_question") {
    ...
    const targetTokenLimitForTranscriptItself =
      availableTokensForUserPayload - qaPreambleTokens - qaPostambleTokens;

    if (targetTokenLimitForTranscriptItself <= 50) {
      encoding.free();
      const headers = new Headers({"Content-Type": "application/json"});
      if (
        encoding.encode(transcriptText).length >
        targetTokenLimitForTranscriptItself
      )
        headers.set("X-Content-Truncated", "true");
      return Response.json(
        {success: false, error: "Not enough tokens for Q&A transcript part."},
        {status: 413, headers}
      );
    }

    const truncationResult = truncateStringToTokenLimit(
      transcriptText,
      targetTokenLimitForTranscriptItself,
      encoding
    );
    ...
    if (userPayloadWasActuallyTruncated)
      transcriptSegmentForLlm += "\n...[TRANSCRIPT SEGMENT TRUNCATED]";

    console.log(
      `[AI Action] Q&A: Original transcript tokens: ${
        encoding.encode(transcriptText).length
      }. Target for segment: ${targetTokenLimitForTranscriptItself}. Actual segment tokens (before marker): ${
        truncationResult.tokenCount
      }.`
    );
    ...
    console.log(
      `[AI Action] Task "${taskType}": Original transcript tokens: ${
        encoding.encode(transcriptText).length
      }. Target for segment: ${availableTokensForUserPayload}. Actual segment tokens (before marker): ${
        truncationResult.tokenCount
      }.`
    );
  ...
  if (userPayloadWasActuallyTruncated) {
    console.warn(
      `[AI Action] Input transcript was definitively truncated for task "${taskType}".`
    );
  }

  const finalUserMessageActualTokens = encoding.encode(
    finalUserMessageContentForLlm
  ).length;
  const totalCalculatedInputTokens =
    systemPromptTokens + finalUserMessageActualTokens;

  console.log(
    `[AI Action] FINAL TOKEN CHECK - Total Input: ${totalCalculatedInputTokens} (System: ${systemPromptTokens}, User: ${finalUserMessageActualTokens}). Model: "${modelToUse}", Context Limit: ${modelContextLimit}, Reserved for Output/TPM: ${TOKENS_RESERVED_FOR_OUTPUT_AND_TPM_BUFFER}`
  );
  ...
  try {
    const operation = async () => {
      console.log(
        `[AI Action] Attempting Groq chat completion stream. Model: "${modelToUse}", Task: "${taskType}". User message tokens: ${finalUserMessageActualTokens}.`
      );
    ...
    if (userPayloadWasActuallyTruncated) {
      ...
      console.log(
        "[AI Action] Setting X-Content-Truncated header to true for successful stream."
      );
    }
    ...
    const readableWebStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          console.error(
            "Stream processing error in ReadableStream:",
            streamError
          );
    ...
    // This catch block now primarily handles errors if retryWithBackoff itself gives up
    console.error(
      `[AI Action] Error after all retries for task "${taskType}" with Groq:`,
      error
    );
    ...
    let statusCode = 500;
    const errorCode =
      (error as any).code || ((error as any).cause as any)?.code;
    ...
    if (error instanceof Groq.APIConnectionTimeoutError) {
      ...
      if (
        ...
        errorMessage = `Groq API Error (Status: ${error.status || "N/A"}): ${
          error.message
        }`;
      }
      console.error(
        "[AI Action] Full Groq.APIError object details:",
