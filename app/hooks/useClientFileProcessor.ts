...
export function useClientFileProcessor({
  ...
  const processFile = useCallback(
    async (file: File, mode: TranscriptionMode, isDirectAudio: boolean) => {
      ...
      const needsConversionOrExtraction =
        !isDirectAudio || (isDirectAudio && file.type !== "audio/opus");
      if (needsConversionOrExtraction) {
        if (
          !ffmpegFromProp ||
          (typeof ffmpegFromProp.isLoaded === "function" &&
            !ffmpegFromProp.isLoaded())
        ) {
          onError(
      ...
      if (isDirectAudio) {
        console.log(
          "[ClientProc] Processing direct audio file:",
          originalFileName
        );
        onStagesUpdate([
          {
        ...
        if (file.type === "audio/opus") {
          ...
          try {
            const inputFileName = `input.${
              originalFileName.split(".").pop()?.toLowerCase() || "audio"
            }`;
            const outputFileName = `output.opus`;

            await ffmpegFromProp!.writeFile(
              inputFileName,
              await fetchFile(file)
            ); // Non-null assertion as it's checked
            patch("audio_prep", {
              progress: 0.3,
              subText: "File loaded into FFmpeg...",
            });
            ...
            console.log(
              `[ClientProc] Running FFmpeg for audio conversion: ffmpeg ${conversionArgs.join(
                " "
              )}`
            );

            let lastReportedProgress = 0.3;
            ffmpegFromProp!.on("progress", (pEvent) => {
              // Non-null assertion
              if (
                "progress" in pEvent &&
                pEvent.progress > 0 &&
            ...
            patch("audio_prep", {
              progress: 0.9,
              subText: "Conversion complete, reading file...",
            });
            ...
            if (
              ffmpegFromProp!.FS &&
              typeof ffmpegFromProp!.FS === "function"
            ) {
              // Non-null assertion
              try {
                ...
                console.warn("FFmpeg FS unlink error:", fsErr);
              }
            }
            patch("audio_prep", {
              progress: 1,
              ...
              isComplete: true,
              isIndeterminate: false,
              label: "Audio converted to Opus",
            });
            ffmpegFromProp!.on("progress", () => {}); // Clear progress handler
          } catch (e: unknown) {
            const msg = e instanceof Error ? e.message : String(e);
            onError(
              `FFmpeg audio conversion failed: ${msg}`,
              originalFileName,
              originalFileSizeMB
            );
            if (ffmpegFromProp && typeof ffmpegFromProp.on === "function")
              ffmpegFromProp.on("progress", () => {}); // Ensure progress handler is cleared on error too
            setBusy(false);
            return;
          }
        }
      } else {
        // It's a video file, proceed with FFmpeg audio extraction
        // ffmpegFromProp must be loaded and ready here due to the check at the start of the function.
        console.log(
          "[ClientProc] Extracting audio from video file:",
          originalFileName
        );
        onStagesUpdate([
          {
            ...
            isComplete: false,
            isIndeterminate: false,
            subText: "Using FFmpeg in browser...",
          },
          {
            name: "groq",
            label: "AI Transcribing…",
      ...
      patch("groq", {
        isActive: true,
        isIndeterminate: true,
        subText: `Processing using Groq's ${TRANSCRIPTION_MODEL_DISPLAY_NAMES} model`,
      });

      const formData = new FormData();
      const blobFileName =
        audioBlob.type === "audio/opus" ? "audio.opus" : originalFileName;
      formData.append("audioBlob", audioBlob, blobFileName);

      console.log(
        `[ClientProc] Sending audio to transcribeAudioAction. Type: ${audioBlob.type}, Name: ${blobFileName}`
      );
      ...
      if (!res.success || !res.data) {
        ...
        onError(
          res.error ?? "Transcription failed",
          blobFileName,
      ...
      patch("groq", {
        ...
        isActive: false,
        isComplete: true,
        subText: `Processed with Groq's ${TRANSCRIPTION_MODEL_DISPLAY_NAMES} model`,
      });
    ...
    [
      ...
      onError,
      onStagesUpdate,
    ] // Added ffmpegFromProp to dependency array
  );
