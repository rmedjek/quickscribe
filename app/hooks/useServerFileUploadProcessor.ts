...
export function useServerFileUploadProcessor({
  ...
  const processFile = useCallback(
    async (file: File, mode: TranscriptionMode, isDirectAudio: boolean) => {
      ...
      if (isDirectAudio) {
        console.log(
          "[ServerProc] Processing direct audio file on server:",
          originalFileName
        );
        onStagesUpdate([
          // Simplified stages for direct audio
          {
            name: "upload_audio",
            ...
            subText: "Preparing audio for transcription...",
          },
          {
            name: "groq",
        ...
        patch("upload_audio", {
          ...
          isActive: false,
          isComplete: true,
          label: "Audio ready for transcription",
        });
      } else {
        // It's a video file, use existing server-side FFmpeg extraction logic
        console.log(
          "[ServerProc] Extracting audio from video file on server:",
          originalFileName
        );
        onStagesUpdate([
          {
            name: "extract_server",
            label: "Uploading & Extracting Audio (Server)",
            progress: 0,
            isActive: true,
            isComplete: false,
            isIndeterminate: true,
            subText: "Server is processing video...",
          },
          {
            name: "groq",
            label: "AI Transcribing…",
        ...
        patch("extract_server", {
          isIndeterminate: false,
          progress: 1,
          isActive: false,
          isComplete: true,
          label: "Audio Extracted (Server)",
        });
      }
      ...
      formDataForTranscription.append(
        "audioBlob",
        audioBlobForGroq,
        audioFileNameForGroq
      );

      console.log(
        `[ServerProc] Sending audio to transcribeAudioAction. Type: ${audioBlobForGroq.type}, Name: ${audioFileNameForGroq}`
      );
      const transcriptionResult = await transcribeAudioAction(
      ...
      if (!transcriptionResult.success || !transcriptionResult.data) {
        ...
        onError(
          ...
          audioFileNameForGroq,
          (audioBlobForGroq.size / (1024 * 1024)).toFixed(2)
        );
        setBusy(false);
        return;
      }
